...
//				Toast.makeText(getActivity(), v.getTag().toString(), Toast.LENGTH_LONG).show();
			}
		}
	};
	...
	private OnClickListener chatListener = new OnClickListener() {
		...
		public void onClick(View v) {
			if (LinphoneActivity.isInstanciated()){
				LinphoneActivity.instance().displayChatMessage(v.getTag().toString());
//				Toast.makeText(getActivity(), v.getTag().toString(), Toast.LENGTH_LONG).show();
			}
		}
	};
	...
	public void setPhonePrice(String number, String price){
		TableLayout controls = (TableLayout) view.findViewById(R.id.controls);
		int nbrview = controls.getChildCount();
		for(int i=0; i< nbrview; i++){
			View view = controls.getChildAt(i);
			String n = (String)view.getTag();
			if(n != null && n.equals(number)){
				String[] prices = price.split("\\|",-1);
//				Toast.makeText(getActivity(), number + " "+ price+ " "+ prices[0]+ " "+prices[1] , Toast.LENGTH_LONG).show();
				((TextView) view.findViewById(R.id.price_call)).setText(prices[0]);
				((TextView) view.findViewById(R.id.price_tchat)).setText(prices[1]);
			}
		}
//		Toast.makeText(getActivity(), number + " "+ price , Toast.LENGTH_LONG).show();
	}
	...
	private void displayContact(LayoutInflater inflater, View view) {
		...
		for (String numberOrAddress : contact.getNumerosOrAddresses()) {
			...
			if(numberOrAddress.startsWith("00") | numberOrAddress.startsWith("+"))
				new GetpriceTask(getActivity().getBaseContext(), true).execute(numberOrAddress, getActivity().getString(R.string.applang));
