...
import org.linphone.HistorySimpleFragment.GetHistoryTask;
...
public class DialerFragment extends Fragment {
	public static String TAG = "DialerFragment";
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, 
		...
		mCountryFlag = (ImageView) view.findViewById(R.id.adress_flag);
		mCountryFlag.setVisibility(View.INVISIBLE);
		mPrice = (TextView)view.findViewById(R.id.countryprice);

		mAddress.setCountryCodeListener(new AddressText.CountryCodeLenthListener() {
			
			@Override
			public void gotCountryCode(boolean istrue, String text) {
				if(istrue && (text.startsWith("+") || text.startsWith("00")) ){
					Context c = DialerFragment.this.getActivity();
//					if(Utils.getStringFromPreference(c, Config.LOCK_PRICE_TASK)==null || Utils.getStringFromPreference(c, Config.LOCK_PRICE_TASK).equalsIgnoreCase("no")){
					
					if(getpricetask == null){
						getpricetask = new GetpriceTask(getActivity());
						getpricetask.execute(text, c.getString(R.string.applang));
						mCountryFlag.setVisibility(View.INVISIBLE);
						mCountryFlag.setDrawingCacheEnabled(false);
						mPrice.setText("...");
					}else{
...
//							Toast.makeText(getActivity(), getActivity().getString(R.string.already_running), Toast.LENGTH_LONG).show();
						}else if(getpricetask.getStatus() == AsyncTask.Status.FINISHED){
							getpricetask = new GetpriceTask(getActivity());
							getpricetask.execute(text, c.getString(R.string.applang));
							mCountryFlag.setVisibility(View.INVISIBLE);
							mCountryFlag.setDrawingCacheEnabled(false);
...
//						new GetpriceTask(c).execute(text, c.getString(R.string.applang));
...
//					}
	...
	public void setcallPrice(String response){
		String text = "";		
		try {
			JSONObject json = (JSONObject) new JSONObject(response);
			text = json.getString(Config.JSON_COUNTRYNAME) + " : " + json.getString(Config.JSON_PRICE) + " €/min";
			String flagname = json.getString(Config.JSON_FILENAME).trim();
			mCountryFlag.setImageDrawable(null);
			mCountryFlag.setDrawingCacheEnabled(false);
			mCountryFlag.setImageResource(Utils.inconMAp.get(flagname));
			mCountryFlag.setVisibility(View.VISIBLE);
			mPrice.setText(text);
...
//			Utils.sHowDialog(getActivity(), "error loading file",e.toString());
	...
	public void setErrorNumber(String response){
		mPrice.setText(response);
		mCountryFlag.setVisibility(View.INVISIBLE);
		
	}

	public void setErrorNetwork(String response){
		mPrice.setText(response);
		mCountryFlag.setVisibility(View.INVISIBLE);
	}
